...
**Current version:** v1.23.1 | **Total commits:** 130+ | **Test suite:** 1,152 tests (three suites, AI accuracy 100%)
...
generate_summary step added to Tutorials #1, #3, #4 with playback handler. Copy to Clipboard
button in sticky toolbar. 6 new endpoint tests. 1,152 tests.
...
save, explain, export) and #9 (Hospital Readmissions HRRP — 975 rows, 15 steps, two-column
reference JOIN on condition+year, penalty trend, system deterioration). 12 cases, 8 domains.
...
SOW stress test complete. CONTEXT_AW.md session log and documentation audit entries added.

**v1.21.0** | 2026-04-07
Privacy Mode toggle in Settings panel. key_manager.py migrated to JSON config format.
5 prompt builders restrict to schema+stats when enabled. Reference Guide rewritten with
per-endpoint AI transmission disclosure. 8 new privacy mode tests. AI audit verified.
